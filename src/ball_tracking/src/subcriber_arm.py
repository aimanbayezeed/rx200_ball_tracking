...
def plan_trajectory_async():
    threading.Thread(target=plan_trajectory).start()

def plan_trajectory():
    global bot, detected, box_row, desired_z
    if bot is None:
        rospy.logerr('Bot is not initialized!')
        return
    
    error_z = desired_z - box_row
    print("box_row traj: ", box_row)
...
def arm_control_callback_row(data):
    ...
    box_row = data.data
    print("box_row main:", box_row)  # Print box_row after updating
    
    if detected:
        plan_trajectory_async()  # Call the asynchronous version
        print("traj")
